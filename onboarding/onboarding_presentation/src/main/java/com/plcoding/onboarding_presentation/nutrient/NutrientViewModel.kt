...
class NutrientViewModel @Inject constructor(
    ...
    fun onEvent(event: NutrientGoalTypeEvent) {
        when (event) {
            NutrientGoalTypeEvent.OnNextClick -> {
            ...
            is NutrientGoalTypeEvent.OnCarbRatioEnter -> {
                state = state.copy(
                    carbsRatio = filterOutDigits(event.ratio)
                )
            }
            is NutrientGoalTypeEvent.OnProteinRatioEnter -> {
                state = state.copy(
                    proteinRatio = filterOutDigits(event.ratio)
                )
            }
            is NutrientGoalTypeEvent.OnFatRatioEnter -> {
                state = state.copy(
